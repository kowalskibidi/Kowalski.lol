...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("kowalski.tuff | Security : You are running another script, please disable it and execute again")
    end
end
...
local window = library:window({name = "kowalski", suffix = ".tuff", gameInfo = string.format("kowalski : %s", Game_Name:lower())})
