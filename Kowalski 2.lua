...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("kowalski.lol | Security : You are running another script, please disable it and execute again")
    end
end
...
getgenv().library = {
    directory = "kowalski.lol_remastered",
    folders = {
        "/fonts",
        "/configs",
        "/assets"
...
local window = library:window({name = "bronx", suffix = ".lol", gameInfo = string.format("kowalski.lol : %s", Game_Name:lower())})
